...
{
    pub fn new(client: T, parent_path: &str, fabric_id: &str) -> Self {
        Self {
            client,
            path: format!("{}/{}", parent_path, fabric_id),
        }
    }
}
impl<T> FabricIdClient<T>
...
impl PostParams {
    pub fn new(node_id: String, protocol: Protocol) -> Self {
        Self {
            node_id,
            protocol,
            digest: ::std::default::Default::default(),
...
where
    T: crate::client::Client,
{
    pub fn node_id(&self, node_id: &str) -> node_id::NodeIdClient<T> {
        node_id::NodeIdClient::<T>::new(self.client.clone(), &self.path, node_id)
    }
}
