...
impl GetOutputItems {
    pub fn new(digest: DigestStr, group: GroupStr) -> Self {
        Self {
            digest,
            group,
...
impl crate::types::bounded_string::BoundedString for GroupStr {
    const MIN_LENGTH: Option<usize> = Some(2usize);
    const MAX_LENGTH: Option<usize> = Some(18usize);
    const DEFAULT: Option<&'static str> = None::<&'static str>;
    const PATTERN: Option<&'static str> = Some("[A-Za-z][A-Za-z0-9\\-\\_]+");
    const TYPE_DESCRIPTION: &'static str =
        "a string with pattern r\"[A-Za-z][A-Za-z0-9\\-\\_]+\" and length between 2 and 18";
    fn get_value(&self) -> &str {
        &self.value
    }
    fn new(value: String) -> Result<Self, crate::types::bounded_string::BoundedStringError> {
...
impl std::convert::TryFrom<String> for GroupStr {
    type Error = crate::types::bounded_string::BoundedStringError;
    fn try_from(value: String) -> Result<Self, Self::Error> {
        crate::types::bounded_string::BoundedString::new(value)
    }
}
impl ::serde::Serialize for GroupStr {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: ::serde::Serializer,
    {
...
impl<'de> ::serde::Deserialize<'de> for GroupStr {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: ::serde::Deserializer<'de>,
    {
